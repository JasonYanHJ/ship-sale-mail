    ...
    async def _process_auto_forward(self, email_id: int, parsed_email: Dict[str, Any], attachment_models: List[AttachmentModel]):
        ...
        if parsed_email.get('from_system') == 'Vship':
            ...
            if parsed_email.get('type') == 'REMINDER':
                ...
                match = re.search(
                    r'Memo_(\d{4}-\d{5})', parsed_email.get('subject'))

                if match:
                    async with db_manager.get_read_connection() as conn:
                        async with conn.cursor(aiomysql.DictCursor) as cursor:
                            sql = "SELECT id from emails WHERE from_system = 'Vship' AND `type` = 'RFQ' AND subject LIKE %s"
                            await cursor.execute(sql, f'%RFQ_{match.group(1)}%')
                            email = await cursor.fetchone()
                            logger.debug(email)
                            if email:
                                original_email_id = email['id']
